...
class Knearest:
    ...
    def majority(self, item_indices):
        ...
        #counter dict sub to keep track of count
        #get labels
        ...
        cnt = Counter(labels)
        #print cnt
        #get the counts, find the max, find out how many there are (so we know if we need to use median)
        values = cnt.values()
        #print values
        index_max = max(values)
        max_total = values.count(index_max)
        #print max_total
        most_common = cnt.most_common(max_total)
        common_index = [i[0] for i in most_common]
        if max_total > 1:
            return numpy.median(common_index)
        else:
